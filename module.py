...
def discriminator_midinet(image, options, reuse=False, name="discriminator"):
    with tf.compat.v1.variable_scope(name):
        ...
        h0 = lrelu(batch_norm(conv2d(image, options.df_dim,
                                     name='d_h0_conv'), name='d_h0_conv_bn'))
        # h0 is (128 x 128 x self.df_dim)
        h1 = lrelu(batch_norm(conv2d(h0, options.df_dim,
                                     name='d_h1_conv'), name='d_h1_conv_bn'))
        # h1 is (64 x 64 x self.df_dim*2)
        h2 = lrelu(batch_norm(conv2d(h1, options.df_dim,
                                     name='d_h2_conv'), name='d_h2_conv_bn'))
        # h2 is (32x 32 x self.df_dim*4)
        h3 = linear(tf.reshape(h2, [options.batch_size, -1]),
                    options.df_dim * 16, scope='d_h3_linear')
...
def generator_midinet(image, options, reuse=False, name='generator'):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        h0 = tf.nn.relu(batch_norm(
            linear(image, options.df_dim * 16, 'g_h0_lin'), name='g_h0_lin_bn'))
        h1 = tf.nn.relu(batch_norm(
            linear(h0, options.df_dim * 8, 'g_h1_lin'), name='g_h1_lin_bn'))
        h1 = tf.reshape(h1, [options.batch_size, 2, 1, options.gf_dim * 4])
        h5 = tf.nn.relu(batch_norm(deconv2d(h1, options.df_dim * 2,
                                            [4, 1], [4, 1], name='g_h5_conv'), name='g_h5_conv_bn'))
        h6 = tf.nn.relu(batch_norm(deconv2d(h5, options.df_dim * 2,
                                            [4, 1], [4, 1], name='g_h6_conv'), name='g_h6_conv_bn'))
        h7 = tf.nn.relu(batch_norm(deconv2d(h6, options.df_dim * 2,
                                            [4, 1], [4, 1], name='g_h7_conv'), name='g_h7_conv_bn'))
        h8 = tf.nn.tanh(batch_norm(deconv2d(h7, options.output_c_dim, [1, 64], [
                        1, 64], name='g_h8_conv'), name='g_h8_conv_bn'))
...
def discriminator_musegan_bar(input, reuse=False, name='discriminator_bar'):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        # conv
        h0 = lrelu(conv2d(input, 128, [1, 12], [
                   1, 12], padding='VALID', name='d_h0_conv'))
        h1 = lrelu(conv2d(h0, 128, [1, 7], [1, 7],
                          padding='VALID', name='d_h1_conv'))
        h2 = lrelu(conv2d(h1, 128, [2, 1], [2, 1],
                          padding='VALID', name='d_h2_conv'))
        h3 = lrelu(conv2d(h2, 128, [2, 1], [2, 1],
                          padding='VALID', name='d_h3_conv'))
        h4 = lrelu(conv2d(h3, 256, [4, 1], [2, 1],
                          padding='VALID', name='d_h4_conv'))
        h5 = lrelu(conv2d(h4, 512, [3, 1], [2, 1],
                          padding='VALID', name='d_h5_conv'))

        # linear
        h6 = tf.reshape(
            h5, [-1, np.product([s.value for s in h5.get_shape()[1:]])])
...
def discriminator_musegan_phase(input, reuse=False, name='discriminator_phase'):
    with tf.compat.v1.variable_scope(name):
        ...
        # conv
        h0 = lrelu(conv2d(tf.expand_dims(input, axis=2), 512, [
                   2, 1], [1, 1], padding='VALID', name='d_h0_conv'))
        h1 = lrelu(conv2d(h0, 128, [3, 1], [3, 1],
                          padding='VALID', name='d_h1_conv'))

        # linear
        h2 = tf.reshape(
            h1, [-1, np.product([s.value for s in h1.get_shape()[1:]])])
...
def generator_musegan_bar(input, output_dim, reuse=False, name='generator_bar'):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        h7 = deconv2d(h6, output_dim, [1, 12], [
                      1, 12], padding='VALID', name='g_h7_deconv')
...
def discriminator(image, options, reuse=False, name="discriminator"):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        # (32, 42, 64)
        h1 = lrelu(instance_norm(
            conv2d(h0, options.df_dim * 4, name='d_h1_conv'), 'd_bn1'))
        # (16, 21, 256)
        h4 = conv2d(h1, 1, s=1, name='d_h3_pred')
        # (16, 21, 1)
        return h4
...
def generator_unet(image, options, reuse=False, name="generator"):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        e2 = instance_norm(
            conv2d(lrelu(e1), options.gf_dim*2, name='g_e2_conv'), 'g_bn_e2')
        # e2 is (64 x 64 x self.gf_dim*2)
        e3 = instance_norm(conv2d(lrelu(e2), options.gf_dim*4,
                                  s=3, name='g_e3_conv'), 'g_bn_e3')
        # e3 is (32 x 32 x self.gf_dim*4)
        e4 = instance_norm(conv2d(lrelu(e3), options.gf_dim*8,
                                  s=[2, 1], name='g_e4_conv'), 'g_bn_e4')
        # e4 is (16 x 16 x self.gf_dim*8)
        e5 = instance_norm(conv2d(lrelu(e4), options.gf_dim*8,
                                  s=[2, 1], name='g_e5_conv'), 'g_bn_e5')
        # e5 is (8 x 8 x self.gf_dim*8)
        e6 = instance_norm(conv2d(lrelu(e5), options.gf_dim*8,
                                  s=[2, 7], name='g_e6_conv'), 'g_bn_e6')
        # e6 is (4 x 4 x self.gf_dim*8)
        e7 = instance_norm(conv2d(lrelu(e6), options.gf_dim*8,
                                  s=[2, 1], name='g_e7_conv'), 'g_bn_e7')
        # e7 is (2 x 2 x self.gf_dim*8)
        e8 = instance_norm(conv2d(lrelu(e7), options.gf_dim*8,
                                  s=[2, 1], name='g_e8_conv'), 'g_bn_e8')
...
def generator_resnet(image, options, reuse=False, name="generator"):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        def residule_block(x, dim, ks=3, s=1, name='res'):
            ...
            y = instance_norm(
                conv2d(y, dim, ks, s, padding='VALID', name=name+'_c1'), name+'_bn1')
            ...
            y = instance_norm(
                conv2d(y, dim, ks, s, padding='VALID', name=name+'_c2'), name+'_bn2')
        ...
        c1 = relu(instance_norm(conv2d(c0, options.gf_dim, 7, 1,
                                       padding='VALID', name='g_e1_c'), 'g_e1_bn'))
        # c1 is (# of images * 256 * 256 * 64)
        c2 = relu(instance_norm(
            conv2d(c1, options.gf_dim*2, 3, 2, name='g_e2_c'), 'g_e2_bn'))
        # c2 is (# of images * 128 * 128 * 128)
        c3 = relu(instance_norm(
        ...
        d1 = relu(instance_norm(
            deconv2d(r10, options.gf_dim*2, 3, 2, name='g_d1_dc'), 'g_d1_bn'))
        # d1 is (# of images * 128 * 128 * 128)
        d2 = relu(instance_norm(
            deconv2d(d1, options.gf_dim, 3, 2, name='g_d2_dc'), 'g_d2_bn'))
        # d2 is (# of images * 256 * 256 * 64)
        d3 = tf.pad(d2, [[0, 0], [3, 3], [3, 3], [0, 0]], "REFLECT")
        # After padding, (# of images * 262 * 262 * 64)
        pred = tf.nn.sigmoid(
            conv2d(d3, options.output_c_dim, 7, 1, padding='VALID', name='g_pred_c'))
...
def discriminator_classifier(image, options, reuse=False, name="discriminator"):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        h0 = lrelu(conv2d(image, options.df_dim, [
                   1, 12], [1, 12], name='d_h0_conv'))
        # h0 is (64 x 7 x self.df_dim)
        h1 = lrelu(instance_norm(conv2d(h0, options.df_dim * 2,
                                        [4, 1], [4, 1], name='d_h1_conv'), 'd_bn1'))
        # h1 is (16 x 7 x self.df_dim*2)
        h2 = lrelu(instance_norm(conv2d(h1, options.df_dim * 4,
                                        [2, 1], [2, 1], name='d_h2_conv'), 'd_bn2'))
        # h2 is (8 x 7 x self.df_dim*4)
        h3 = lrelu(instance_norm(conv2d(h2, options.df_dim * 8,
                                        [8, 1], [8, 1], name='d_h3_conv'), 'd_bn3'))
        # h3 is (1 x 7 x self.df_dim*8)
        h4 = conv2d(h3, 2, [1, 7], [1, 7], name='d_h3_pred')
        # h4 is (1 x 1 x 2)
...
def PhraseGenerator(in_tensor, output_dim, reuse=False, name='generator'):
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse):
        h0 = tf.reshape(in_tensor, tf.stack(
            [-1, 1, 1, in_tensor.get_shape()[1]]))
...
def BarGenerator(in_tensor, output_dim, reuse=False, name='generator'):
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse):
        ...
        h0 = tf.reshape(in_tensor, tf.stack(
            [-1, 1, 1, in_tensor.get_shape()[1]]))
        h0 = relu(batch_norm(deconv2d_musegan(h0, [1, 1], 1024, kernels=[
                  1, 1], strides=[1, 1], name='h0'), 'h0_bn'))

        h1 = tf.reshape(h0, [-1, 2, 1, 512])
        h1 = relu(batch_norm(deconv2d_musegan(h1, [4, 1], 512, kernels=[
                  2, 1], strides=[2, 1], name='h1'), 'h1_bn'))

        h2 = relu(batch_norm(deconv2d_musegan(h1, [8, 1], 256, kernels=[
                  2, 1], strides=[2, 1], name='h2'), 'h2_bn'))

        h3 = relu(batch_norm(deconv2d_musegan(h2, [16, 1], 256, kernels=[
                  2, 1], strides=[2, 1], name='h3'), 'h3_bn'))

        h4 = relu(batch_norm(deconv2d_musegan(h3, [32, 1], 128, kernels=[
                  2, 1], strides=[2, 1], name='h4'), 'h4_bn'))

        h5 = relu(batch_norm(deconv2d_musegan(h4, [96, 1], 128, kernels=[
                  3, 1], strides=[3, 1], name='h5'), 'h5_bn'))

        h6 = relu(batch_norm(deconv2d_musegan(h5, [96, 7], 64, kernels=[
                  1, 7], strides=[1, 1], name='h6'), 'h6_bn'))

        h7 = deconv2d_musegan(h6, [96, 84], output_dim, kernels=[
                              1, 12], strides=[1, 12], name='h7')
...
def BarDiscriminator(in_tensor, reuse=False, name='discriminator'):
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse):
        ...
        # conv
        h0 = lrelu(conv2d_musegan(in_tensor, 128, kernels=[
                   1, 12], strides=[1, 12], name='h0'))
        h1 = lrelu(conv2d_musegan(h0, 128, kernels=[
                   1, 7], strides=[1, 7], name='h1'))
        h2 = lrelu(conv2d_musegan(h1, 128, kernels=[
                   2, 1], strides=[2, 1], name='h2'))
        h3 = lrelu(conv2d_musegan(h2, 128, kernels=[
                   2, 1], strides=[2, 1], name='h3'))
        h4 = lrelu(conv2d_musegan(h3, 256, kernels=[
                   4, 1], strides=[2, 1], name='h4'))
        h5 = lrelu(conv2d_musegan(h4, 512, kernels=[
                   3, 1], strides=[2, 1], name='h5'))
...
def PhraseDiscriminator(in_tensor, reuse=False, name='discriminator'):
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse):
        ...
        # conv
        h0 = lrelu(conv2d_musegan(tf.expand_dims(in_tensor, axis=2),
                                  512, kernels=[2, 1], strides=[1, 1], name='h0'))
        h1 = lrelu(conv2d_musegan(h0, 128, kernels=[
                   3, 1], strides=[3, 1], name='h1'))
