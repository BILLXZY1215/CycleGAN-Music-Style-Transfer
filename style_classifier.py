...
class Classifer(object):
    ...
    def _build_model(self):
        ...
        self.origin_train = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.time_step, self.pitch_range,
                                                                  self.input_c_dim])
        self.label_train = tf.compat.v1.placeholder(
            tf.float32, [self.batch_size, 2])
        self.origin_test = tf.compat.v1.placeholder(
        ...
        self.D_test = self.discriminator(
            self.origin_test, self.options, True, name='classifier')
        ...
        self.accuracy_test = tf.reduce_mean(
            tf.cast(self.correct_prediction_test, tf.float32))

        # test midi
        self.test_midi = tf.compat.v1.placeholder(
            tf.float32, [None, self.time_step, self.pitch_range, self.input_c_dim])
        self.test_result = self.discriminator(
            self.test_midi, self.options, True, name='classifier')
    ...
    def train(self, args):
        ...
        label_test = np.array(label_test).astype(
            np.float32).reshape(len(label_test), 2)
        ...
        for epoch in range(args.epoch):
            ...
            for idx in range(batch_idx):
                ...
                batch_label = np.array(batch_label).astype(
                    np.float32).reshape(len(batch_label), 2)
            ...
            print('epoch:', epoch, 'testing accuracy:',
                  accuracy_test, 'loss:', d_loss)
    ...
    def test(self, args):
        ...
        for idx in range(len(sample_files)):
            ...
            origin_transfer_diff = np.abs(
                test_result_origin - test_result_transfer)
            content_diff = np.mean(
                (sample_origin * 1.0 - sample_transfer * 1.0)**2)
            ...
            if args.which_direction == 'AtoB':
                ...
                count_origin += 1 if np.argmax(
                    test_result_origin[0]) == 0 else 0
                count_transfer += 1 if np.argmax(
                    test_result_transfer[0]) == 0 else 0
                ...
                # for the accuracy calculation
                count_origin += 1 if np.argmax(
                    test_result_origin[0]) == 1 else 0
                count_transfer += 1 if np.argmax(
                    test_result_transfer[0]) == 1 else 0
    ...
    def test_famous(self, args):
        ...
        for idx in range(song_t.shape[0]):
            phrase_o = song_o[idx]
            phrase_o = phrase_o.reshape(
                1, phrase_o.shape[0], phrase_o.shape[1], 1)
            origin = self.sess.run(self.test_result_softmax, feed_dict={
                                   self.test_midi: phrase_o * 2. - 1.})
            phrase_t = song_t[idx]
            phrase_t = phrase_t.reshape(
                1, phrase_t.shape[0], phrase_t.shape[1], 1)
        ...
        print("origin, source:", sum_o_A /
              song_t.shape[0], "target:", sum_o_B / song_t.shape[0])
        print("transfer, source:", sum_t_A /
              song_t.shape[0], "target:", sum_t_B / song_t.shape[0])
