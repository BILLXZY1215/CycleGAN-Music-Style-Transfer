...
class cyclegan(object):
    ...
    def _build_model(self):
        ...
        if self.model != 'base':
            ...
            self.db_all_loss_fake = self.criterionGAN(
                self.DB_fake_sample_all, tf.zeros_like(self.DB_fake_sample_all))
            ...
            self.da_all_loss_fake = self.criterionGAN(
                self.DA_fake_sample_all, tf.zeros_like(self.DA_fake_sample_all))
        ...
        self.testA_ = self.generator(
            self.testB, self.options, True, name='generatorB2A')
        # B - A - B
        self.testA = self.generator(
            self.test_B, self.options, True, name="generatorB2A")
        self.testB_ = self.generator(
            self.testA, self.options, True, name='generatorA2B')
        # to binary
    ...
    def train(self, args):
        ...
        # Learning rate
        self.lr = tf.compat.v1.placeholder(
        ...
        self.g_optim = tf.compat.v1.train.AdamOptimizer(
            self.lr, beta1=args.beta1).minimize(self.g_loss, var_list=self.g_vars)
        ...
        for epoch in range(args.epoch):
            ...
            for idx in range(0, batch_idxs):
                ...
                batch_images = [load_npy_data(batch_file)
                ...
                if self.model == 'base':
                    ...
                    # To feed real_mixed
                    batch_files_mixed = data_mixed[idx *
                                                   self.batch_size:(idx + 1) * self.batch_size]
                    batch_images_mixed = [
                        np.load(batch_file) * 1. for batch_file in batch_files_mixed]
                    batch_images_mixed = np.array(
                        batch_images_mixed).astype(np.float32)
    ...
    def sample_model(self, sample_dir, epoch, idx):
        ...
        dataA = glob('./datasets/{}/train/*.*'.format(self.dataset_A_dir))
        dataB = glob('./datasets/{}/train/*.*'.format(self.dataset_B_dir))
        dataA.sort(key=lambda x: int(os.path.splitext(
            os.path.basename(x))[0].split('_')[-1]))
        dataB.sort(key=lambda x: int(os.path.splitext(
            os.path.basename(x))[0].split('_')[-1]))

        batch_files = list(
            zip(dataA[:self.batch_size], dataB[:self.batch_size]))
        sample_images = [load_npy_data(batch_file)
                         for batch_file in batch_files]
        ...
        save_midis(
            real_A_binary, './{}/A2B/{:02d}_{:04d}_origin.mid'.format(sample_dir, epoch, idx))
        save_midis(
            fake_B_binary, './{}/A2B/{:02d}_{:04d}_transfer.mid'.format(sample_dir, epoch, idx))
        save_midis(
            fake_A__binary, './{}/A2B/{:02d}_{:04d}_cycle.mid'.format(sample_dir, epoch, idx))
        save_midis(
            real_B_binary, './{}/B2A/{:02d}_{:04d}_origin.mid'.format(sample_dir, epoch, idx))
        save_midis(
            fake_A_binary, './{}/B2A/{:02d}_{:04d}_transfer.mid'.format(sample_dir, epoch, idx))
        save_midis(
            fake_B__binary, './{}/B2A/{:02d}_{:04d}_cycle.mid'.format(sample_dir, epoch, idx))
    ...
    def test(self, args):
        ...
        if args.which_direction == 'AtoB':
            sample_files = glob(
                './datasets/{}/test/*.*'.format(self.dataset_A_dir))
        elif args.which_direction == 'BtoA':
            sample_files = glob(
                './datasets/{}/test/*.*'.format(self.dataset_B_dir))
        else:
        ...
        for idx in range(len(sample_files)):
            ...
            sample_npy_re = sample_npy.reshape(
                1, sample_npy.shape[0], sample_npy.shape[1], 1)
            ...
            midi_path_cycle = os.path.join(
                test_dir_mid, '{}_cycle.mid'.format(idx + 1))
            ...
            np.save(os.path.join(npy_path_origin,
                                 '{}_origin.npy'.format(idx + 1)), origin_midi)
            np.save(os.path.join(npy_path_transfer,
                                 '{}_transfer.npy'.format(idx + 1)), fake_midi)
            np.save(os.path.join(npy_path_cycle,
                                 '{}_cycle.npy'.format(idx + 1)), fake_midi_cycle)
